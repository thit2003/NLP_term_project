...
def default_thai_dir() -> str:
    local = Path(__file__).parent / "thai_model"
    return str(local) if local.is_dir() else "./thai_model"
...
def main():
    p = argparse.ArgumentParser(description="Compare Thai vs Multilingual sentiment models")
    p.add_argument("--text", default=None, help="Input text to analyze (optional; will prompt if omitted)")
    p.add_argument("--thai-model", default=os.environ.get("TH_MODEL", default_thai_dir()))
    p.add_argument("--multi-model", default=os.environ.get("MULTI_MODEL", default_multi_dir()))
    args = p.parse_args()

    text = args.text
    if not text:
        print("Enter Thai text (Ctrl+C to exit):")
        text = input("> ").strip()

    th_label, th_conf = predict_one(text, args.thai_model)
    ml_label, ml_conf = predict_one(text, args.multi_model)

    print(f'\nInput: "{text}"')
    print(f"* `Thai Model` Prediction: {th_label.capitalize()} (Confidence: {th_conf:.2f})")
